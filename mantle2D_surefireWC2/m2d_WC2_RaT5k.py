...
if not restart:
    ...
    write, initial_timestep = solver.load_state('checkpoints_RaT5k/checkpoints_RaT5k_s1.h5')

# Analysis
snapshots = solver.evaluator.add_file_handler('snapshots_RaT5k', iter=100, max_writes=50)
...
snapshots_nonlinear = solver.evaluator.add_file_handler('snapshots_nonlinear_RaT5k', iter=10, max_writes=200)
snapshots_nonlinear.add_task(d3.Average((ez @ u)*T, ('x')), name='convective_heat_flux_z')

# Checkpoints
checkpoints = solver.evaluator.add_file_handler('checkpoints_RaT5k', sim_dt=1.0, max_writes=1, mode=file_handler_mode)
