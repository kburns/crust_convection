...
if not restart:
    ...
    write, initial_timestep = solver.load_state('checkpoints_CRa10k/checkpoints_CRa10k_s1.h5')

# Analysis
snapshots = solver.evaluator.add_file_handler('snapshots_CRa10k', iter=100, max_writes=10)
...
snapshots_nonlinear = solver.evaluator.add_file_handler('snapshots_nonlinear_CRa10k', iter=10, max_writes=200)
snapshots_nonlinear.add_task(d3.Average((ez @ u)*T, ('x', 'y')), name='convective_heat_flux_z')

# Checkpoints
checkpoints = solver.evaluator.add_file_handler('checkpoints_CRa10k', sim_dt=max_timestep*1000, max_writes=1, mode=file_handler_mode)
