...
SA_Earth = 5.1e14       # m^2
rho_crust = 2.8e3       # kg/m^3
cont_mass = 1.8e22      # kg

time_scale = D_scale**2 / kappa_scale #real seconds per model second

crustal_losspersecond_air  = air_erosion*SA_Earth*rho_crust/cont_mass  # %/s
crustal_losspersecond_rain = rain_erosion*SA_Earth*rho_crust/cont_mass # %/s

totalC = 115000        # obtained from noerosion case
likely_dt = 6e-5       # obtained from noerosion case

C_losspertimestep_air  = crustal_losspersecond_air*totalC*time_scale*likely_dt/(Nx*Ny)
C_losspertimestep_rain = crustal_losspersecond_rain*totalC*time_scale*likely_dt/(Nx*Ny)

p_scale = 260          # obtained from noerosion case
lapp_scale = 11700     # obtained from noerosion case
slopep_scale = 4.21e6  # obtained from noerosion case 

Kh = 0 # X*C_losspertimestep_air/p_scale
Kc = 0 # X*C_losspertimestep_air/lapp_scale
Ks = 0 # X*C_losspertimestep_rain/slopep_scale
...
# energy equation
problem.add_equation("dt(T) - div(grad_T) + lift(tau_T2) = - u@grad(T) + h*C + E")
# continent
problem.add_equation("dt(C) - (1/Le)*div(grad_C) + lift(tau_C2) = - u@grad(C) - γ*C")
...
# BC: continent flux set by erosion
