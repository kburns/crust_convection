...
problem.add_equation("dt(T) - div(grad_T) + lift(tau_T2) = - u@grad(T) + h*(1-C) + 1")
problem.add_equation("dt(C) - (1/Le)*div(grad_C) + lift(tau_C2) = - u@grad(C) + γ*(1-C)")
problem.add_equation("-div(grad_u) + grad(p) - (RaI*T - RaC*C)*ez + lift(tau_u2) + tau_p2*ex = 0")
problem.add_equation("ez@grad_T(z=0) = 0")
...
# Checkpoints
checkpoints = solver.evaluator.add_file_handler('checkpoints', sim_dt=1.0, max_writes=1, mode=file_handler_mode)
