...
problem = d3.IVP([p, T, C, u, tau_p, tau_Lx, tau_Ly, tau_Lz, tau_T1, tau_T2, tau_C1, tau_C2, tau_u1, tau_u2], namespace=locals())
...
problem.add_equation("-div(grad_u) + grad(p) - (RaT*T - RaC*C)*er + lift(tau_u2) + tau_Lx*rot_x + tau_Ly*rot_y + tau_Lz*rot_z = 0") # this is 3 equations
...
snapshots.add_task(er @ u, name='u_r')
snapshots.add_task(er @ d3.curl(u), name='vorticity_r')

# Horizontally averaged nonlinear diagnostics
snapshots_nonlinear = solver.evaluator.add_file_handler('snapshots_nonlinear', iter=10, max_writes=200)
snapshots_nonlinear.add_task(d3.Average((er @ u)*T, ('phi', 'theta')), name='convective_heat_flux_r')
snapshots_nonlinear.add_task(d3.Integrate((u@u)/2, coords), name='KE_int')

# Checkpoints
checkpoints = solver.evaluator.add_file_handler('checkpoints', sim_dt=max_timestep*1000, max_writes=1, mode=file_handler_mode)
