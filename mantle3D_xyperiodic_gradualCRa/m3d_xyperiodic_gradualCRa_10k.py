...
if not restart:
  ...
  #  C['g'][..., -1] = 0
    ...
    write, initial_timestep = solver.load_state('checkpoints_10k/checkpoints_10k_s1.h5')

# Analysis
snapshots = solver.evaluator.add_file_handler('snapshots_10k', iter=100, max_writes=10, mode=file_handler_mode)
...
snapshots.add_task((ez @ u)*T, name='convective_heat_flux')

# Horizontally averaged nonlinear diagnostics
snapshots_nonlinear = solver.evaluator.add_file_handler('snapshots_nonlinear_10k', iter=10, max_writes=200, mode=file_handler_mode)
snapshots_nonlinear.add_task(d3.Average((ez @ u)*T, ('x', 'y')), name='convective_heat_flux_z')

# Checkpoints
checkpoints = solver.evaluator.add_file_handler('checkpoints_10k', sim_dt=max_timestep*1000, max_writes=1, mode=file_handler_mode)
